...
salta_n(C,M,X,Y,X1,Y1):-
    (pmov(M) -> (not(ocupado(X1,Y1)),(dxy(C,Dx,Dy);(not(ocupado(X1+1,Y1)),dxyi(C,Dx,Dy)));(ocupado(X1,Y1),dxyc(C,Dx,Dy))) ;
...
salta_b(C,M,X,Y,X1,Y1):-
    (pmov(M) -> (not(ocupado(X1,Y1)),(dxyb(C,Dx,Dy);(not(ocupado(X1-1,Y1)),dxyib(C,Dx,Dy)));(ocupado(X1,Y1),dxycb(C,Dx,Dy))) ;
